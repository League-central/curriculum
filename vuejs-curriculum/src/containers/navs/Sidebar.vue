        ...
        <vue-perfect-scrollbar class="scroll" :settings="{ suppressScrollX: true, wheelPropagation: false }">
            <ul class="list-unstyled">
                <li v-for="(item,index) in menuItems" 
                		...
                		<!--  render link to open new window if item has newWindow element  -->
                    <a v-if="item.newWindow" :href="item.to" rel="noopener noreferrer" target="_blank">
                        <i :class="item.icon" />
                        {{ $t(item.label) }}
                    </a>
                    
                    <!--  render item that has sub items and is clickable-->
                    <a v-else-if="item.subs && item.subs.length>0 && item.clickable" @click.prevent="openSideNavSubMenu($event,item)" :href="item.to">
                        <i :class="item.icon" />
                        {{ $t(item.label) }}
                    </a>

                    <!--  render item that has sub items -->
                    <a v-else-if="item.subs && item.subs.length>0 && !item.clickable" @click.prevent="openSubMenu($event,item)" :href="`#${item.to}`">
                        <i :class="item.icon" />
                        {{ $t(item.label) }}
                    </a>
...
import {
    ...
    subHiddenBreakpoint
} from "../../constants/league/config";
import menuItems from "../../constants/league/menu";
...
export default {
    ...
    methods: {
        ...
        openSideNavSubMenu(e,menuItem) {
        	this.openSubMenu(e,menuItem);
        },
